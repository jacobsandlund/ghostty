...
const TaggedPageAllocator = struct {
    ...
    pub fn map(n: usize, alignment: mem.Alignment, tag: VMTag) ?[*]u8 {
        ...
        // We pass null as the address hint and let the kernel pick. The
        // upstream `PageAllocator` maintains a `next_mmap_addr_hint` to
        // encourage contiguous allocations, but in Zig 0.16 that field
        // was made private (renamed to `addr_hint` inside
        // `lib/std/heap/PageAllocator.zig`). If we want similar behavior
        // here we'd need to maintain our own thread-safe hint atomically,
        // mirroring the upstream logic.
        // See thread: https://ampcode.com/threads/T-019dca3f-5eb7-757a-9b2c-bc7eac1a6f2b
